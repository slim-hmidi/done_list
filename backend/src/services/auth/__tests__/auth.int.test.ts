...
describe('Authentication Apis', () => {
  ...
  describe('Signup', () => {
    ...
    it('Should returns an error if password does not contains a number', async () => {
      ...
      expect(response.status).toBe(400);
      expect(response.body.message).toEqual(errorMessages.numberRequired);
    });
    ...
    it('Should returns an error if brithday does not match the format', async () => {
      ...
      expect(response.status).toBe(400);
      expect(response.body.message).toEqual(errorMessages.dateFormatRequired);
    });
    ...
    it('Should returns an error if the user already exists', async () => {
      ...
      expect(response.body.message).toEqual(errorMessages.userExists);
    });
